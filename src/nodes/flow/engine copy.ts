...
export class FlowEngine {
  ...
  private async processNode(
    ...
    // Process leads if we have a leadId
    if (leadId) {
      ...
      if (existingLead?.status === "completed") {
        // Use existing lead data but preserve message data
        nodeState.inputs = {
          //@ts-ignore
          ...existingLead.data?.inputs,
          message: messageData,
        };
        nodeState.outputs = {
          //@ts-ignore
          ...existingLead.data?.outputs,
          message: messageData,
        };
        nodeState.executed = true;

        // Log and process next nodes
        state.logs.push({
          timestamp: new Date(),
          level: "info",
      ...
      const [lead] = await db
        ...
        .values({
          ...
          leadId,
          status: "processing",
          data: { inputs: { ...nodeState.inputs, message: messageData } },
        })
        .onConflictDoUpdate({
    ...
    // Process input edges and preserve message data
    ...
    for (const edge of inputEdges) {
      ...
      if (sourceNodeState.executed && !sourceNodeState.error) {
        ...
        if (sourceOutput !== undefined) {
          ...
          // Preserve message data from source node
          if (sourceOutput.message) {
            nodeState.inputs.message = sourceOutput.message;
          }
        }
      }
    }
